...
doubleElts xs = map (2 *) xs               -- Double each element in the list
...
sumList xs = sum xs                        -- Fold the list using addition and the base case 0
...
takeN' 0 (x:xs) = []                       -- Pattern matching on the list
takeN' n (x:xs) = x : takeN' (n - 1) xs    -- Add the head element and recurse on the tail 
...
skipWhiteSpace ys@(x:xs)                    -- Pattern matching on the string
  | isSpace x = skipWhiteSpace xs           -- If the head element is white space, skip it
  | otherwise = ys                          -- Otherwise, return the string as is
...
-- skipWhiteSpace' cs = dropWhile isSpace cs
-- getWord cs = 
--   where
--     droppedSpace = dropWhile isSpace cs
--     firstWord = takeWhile isNotSpace droppedSpace 
--     rest = dropWhile isNotSpace droppedSpace 


getWord :: String -> (String, String)
getWord cs = getWord' (skipWhiteSpace cs)
  where
...
getLine' (w:ws) width
    | length w <= width =
        let (line, remaining) = getLine' ws (width - length w - 1)
        in  (w ++ " " ++ line, remaining)
        -- If the length of the first word is less than the width, add it to the line and recurse on the remaining words
    | otherwise = ("", w:ws)                -- Otherwise, return an empty string and the list of words
...
leftJustified n s = leftJustified' n (intoWords s)
  where
    leftJustified' _ [] = []
    leftJustified' width text =
        -- Get the first line and the remaining words
        let (line, remaining) = getLine' text width
        -- Add the line to the list and recurse on the remaining words
        in line : leftJustified' width remaining
...
leftJustified' width text =
    -- Get the first line and the remaining words
    let (line, remaining) = getLine' (intoWords text) width
    -- Add the line to the list and recurse on the remaining words
    in line : leftJustified' width (unwords remaining)
...
getWordsInFirstLine :: [String] -> Int -> ([String], [String])
getWordsInFirstLine [] _ = ([], [])                     -- Base case: empty list returns an empty string and an empty list
getWordsInFirstLine (w:ws) width                           
    | length w <= width = 
        let (line, remaining) = getLine' ws (width - length w - 1) 
        in  (w:line, remaining) 
        -- If the length of the first word is less than the width, add it to the line and recurse on the remaining words
...
justifyWordsToLine :: Integer -> [String] -> String
justifyWordsToLine lineLength words = collate words gaps
  where
    
    totalLength = sum (map length words)

    numberOfSpacesToAdd = (lineLength - totalLength) `div` (length words - 1)
    numberWithExtraSpace = lineLength - (totalLength + numberOfSpacesToAdd * (length words - 1))
 
    gaps = createGaps (length words - 1) numberWithExtraSpace

    createGaps 0 _ = []
    createGaps n 0 = (replicate numberOfSpacesToAdd ' '): createGaps (n-1) 0
    createGaps n m = (replicate (1+numberOfSpacesToAdd) ' '): createGaps (n-1) (m-1)

    collate [] _ = ""
    collate (w:ws) (g:gs) = w++g++collate ws gs


-- justifyWordsToLine 10 ["Gordon","is"] = "Gordon  is"
-}
